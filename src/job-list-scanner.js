(() => {
  ...
  const VERSION = "1.5.0";
  if (globalThis.__VSC_JOB_LIST_SCANNER__ === VERSION) {
  ...
  function leftCard(element) {
    if (!visible(element) || element.closest(DETAIL_SELECTOR)) return false;
    const rect = element.getBoundingClientRect();
  ...
  function normalizedCard(element) {
    ...
    return inner && leftCard(inner) ? inner : element;
  }
  ...
  function validCompany(value, jobTitle) {
    return value.length >= 2
      && value.length <= 180
      && value.toLowerCase() !== jobTitle.toLowerCase()
  ...
  function exactTextElement(card, target) {
    ...
    return [...card.querySelectorAll("a,span,p,div")]
      .filter((element) => {
        ...
        return valueLines.length === 1 && valueLines[0].toLowerCase() === key;
      })
  ...
  function company(card, jobTitle) {
    ...
    const titleIndex = cardLines.findIndex((line) => line.toLowerCase() === jobTitle.toLowerCase());
    const candidates = titleIndex >= 0 ? cardLines.slice(titleIndex + 1) : cardLines.slice(1);
    for (const value of candidates) {
      if (!validCompany(value, jobTitle)) continue;
      const element = exactTextElement(card, value);
  ...
  async function scan(force = false) {
    ...
    try {
      ...
      document.querySelectorAll(".vsc-job-card").forEach((element) => {
        if (!leftCard(element)) clearCard(element);
      });

      const cardList = cards();
      status.cardsDetected = cardList.length;
      let extracted = 0;
  ...
  globalThis.addEventListener("vsc:job-list-rescan", () => schedule(true));
